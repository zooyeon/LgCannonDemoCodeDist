...
static int                    PanErrorDefaultThreshold = 75;
static volatile int           PanErrorThreshold = PanErrorDefaultThreshold;
static int                    PanErrorThresholdInc = 5;
static int                    PanErrorThreaholdLimit = 95;
static int                    StableCountLimit = 2;
static void resetPanErrorThreshold();
static int getPanErrorThreshold();
static void increasePanErrorThreshold();
...
static void ReadOffsets(void)
{
   ...
   char xs[100],ys[100];
   int retval=0;
   char panErrDefThreshold[100];
   char panErrThresholdInc[100];
   char panErrThreaholdLimit[100];
   char stableCountLimit[100];
   int pd, pi, pl, sl;
   ...
   retval=fscanf(fp, "%s %d", panErrDefThreshold, &pd);
   retval+=fscanf(fp, "%s %d", panErrThresholdInc, &pi);
   retval+=fscanf(fp, "%s %d", panErrThreaholdLimit, &pl);
   retval+=fscanf(fp, "%s %d", stableCountLimit, &sl);
   if (retval == 8)
   {
    PanErrorDefaultThreshold = pd;
    PanErrorThreshold = pd;
    PanErrorThresholdInc = pi;
    PanErrorThreaholdLimit = pl;
    StableCountLimit = sl;
    printf("Read value for pan threshold\n");
    printf("PanErrorDefaultThreshold %d\n", PanErrorDefaultThreshold);
    printf("PanErrorThresholdInc %d\n", PanErrorThresholdInc);
    printf("PanErrorThreaholdLimit %d\n", PanErrorThreaholdLimit);
    printf("StableCountLimit %d\n", StableCountLimit);
   }
   fclose(fp);
}
//------------------------------------------------------------------------------------------------
   ...
   case TRACKING:
                {
                  ...
                  if (item.match != -1) {
                    ...
                    Pan=Pan-PanError/getPanErrorThreshold();
                    ServoAngle(PAN_SERVO, Pan);

                    TiltError=(item.center.y+yCorrect)-height/2;
                    Tilt=Tilt-TiltError/getPanErrorThreshold();
                    ...
                    Auto->LastPan=Pan;
                    Auto->LastTilt=Tilt;
                    if (Auto->StableCount>StableCountLimit) state=TRACKING_STABLE;
                    else state=TRACKING;
                  }
...
static void resetPanErrorThreshold() {
  PanErrorThreshold = PanErrorDefaultThreshold;;
}

static int getPanErrorThreshold() {
  return PanErrorThreshold;
}

static void increasePanErrorThreshold() {
  if (PanErrorThreshold < PanErrorThreaholdLimit) PanErrorThreshold += PanErrorThresholdInc;
  printf("[Debug] PanErrorThreshold(%d)\n", PanErrorThreshold);
}